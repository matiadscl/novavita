...
function fetch_shopify_orders(string $since): array {
    ...
    return cached_fetch($cache_key, function() use ($since) {
        $endpoint = "orders.json?status=any&created_at_min={$since}T00:00:00-03:00&limit=250";
        $result = shopify_get($endpoint);
        if (!$result || !isset($result['orders'])) return ['summary' => [], 'by_month' => []];

        $all_orders = $result['orders'];
        $summary = ['total_orders' => 0, 'total_revenue' => 0, 'paid_orders' => 0, 'cancelled' => 0, 'refunded' => 0, 'avg_ticket' => 0, 'top_products' => []];
        $by_month = [];
        $product_counts = [];
        ...
        foreach ($all_orders as $o) {
            ...
            foreach ($o['line_items'] ?? [] as $item) {
                ...
                if (!isset($product_counts[$name])) $product_counts[$name] = ['qty' => 0, 'revenue' => 0];
